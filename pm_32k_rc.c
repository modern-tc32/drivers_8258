...
__attribute__((used, section(".text.cpu_sleep_wakeup_32k_rc"))) int cpu_sleep_wakeup_32k_rc(SleepMode_TypeDef sleep_mode, SleepWakeupSrc_TypeDef wakeup_src, unsigned int wakeup_tick) {
    ...
    {
        uint8_t analog2c = (uint8_t)(0x16 | analog2c_high_bits | any | (cmp << 3));
        if ((sleep_mode_retention != 0u) && (wakeup_src == PM_WAKEUP_TIMER)) {
            analog2c = 0x5eu;
        }
        analog_write(0x2c, analog2c);
    }
...

    {
        ...
        if (sleep_mode == DEEPSLEEP_MODE_RET_SRAM_LOW32K) {
...
__attribute__((used, section(".text.pm_long_sleep_wakeup"))) int pm_long_sleep_wakeup(SleepMode_TypeDef sleep_mode, SleepWakeupSrc_TypeDef wakeup_src, unsigned int sleep_duration_us) {
...

    {
        ...
        uint8_t analog2c = (uint8_t)(v2c_base | any | (cmp << 3));
        if (sleep_mode_retention && (wakeup_src == PM_WAKEUP_TIMER)) {
            analog2c = 0x5eu;
        } else if ((sleep_mode == DEEPSLEEP_MODE) && (wakeup_src == PM_WAKEUP_TIMER)) {
            analog2c = 0xdeu;
        }
        analog_write(0x2c, analog2c);
    }
...

    {
        ...
        if (pm_long_suspend) {
            uint32_t base = minus64 + tick_32k_cur;
            uint32_t q = __udivsi3(dt, (uint32_t)tick_32k_calib);
            wake_tick = base - (q << 4);
        } else {
            wake_tick = minus64 + tick_32k_cur - __udivsi3((dt << 4) + (uint32_t)(calib >> 1), (uint32_t)tick_32k_calib);
        }
    ...
    reg_system_tick_mode = 0x20;
    if ((analog_read(areg_wakeup_status) & WAKEUP_STATUS_ALL) == 0u) {
        sleep_start();
    }
